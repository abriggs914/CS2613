  ...
  (if (eq? dates '())
      ...
      (map (lambda (a) (seconds->date a))
           (map (lambda (x) (if (> (date-time-zone-offset (seconds->date x)) 0)
                                ...
                                (+ x (date-time-zone-offset (seconds->date (- x (* 24 3600)))))))
                (sort
...
(define a (rfc2822->date "Sun, 16 Oct 2011 23:12:02 -0500"))
(define b (rfc2822->date "Mon, 17 Oct 2011 04:15:12 +0000"))
(define c (date*->seconds a))
(define d (date*->seconds b))
(define e (date-time-zone-offset a))
(define f (date-time-zone-offset b))
(define g (- c e))
(define h (- d f))
(define i (+ c e))
(define j (+ d f))
...
;> (seconds->date 1536026264)
;(date* 44 57 22 3 9 2018 1 245 #t -10800 0 "ADT")
